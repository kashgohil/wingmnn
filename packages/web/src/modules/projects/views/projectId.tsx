...
export function ProjectId() {
  ...
  function actions(column: KanbanColumn) {
    const status = find(workflowStatuses, (item) => item.id === column.id);
    return (
      <div className="flex items-center gap-1">
        <IconButton
          ...
          className="p-2"
          onClick={() =>
            ProjectActions.openDialog(ProjectDialog.CREATE_TASK, status)
          }
        />
        <IconButton icon={Search} iconProps={{ size: 20 }} className="p-2" />
      </div>
    );
  }
      ...
      <div className="flex flex-col gap-1 text-center p-4 pb-0 relative">
        <Link to={BaseRoutes.PROJECTS} className="absolute left-4 top-5">
          <Button
            size="sm"
            variant="secondary"
            className="flex items-center group/back p-2 rounded-lg cursor-pointer gap-0 hover:gap-1"
          >
