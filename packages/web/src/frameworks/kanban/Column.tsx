...
export function Column({ column, dragState, onPointerDown, onPointerMove, onPointerUp }: ColumnProps) {
	...
	return (
		<motion.div
			...
			className={cx(
				"min-w-80 rounded-lg p-2 h-full bg-gradient-to-b from-accent/5 to-transparent",
				isSourceColumn && isDragging ? "opacity-75" : ""
			)}
			style={{ "--accent": metadata?.color ?? undefined, "--accent-text": metadata?.color ?? undefined } as TSAny}
			onPointerMove={isDragging ? onPointerMove : undefined}
			onPointerUp={isDragging ? onPointerUp : undefined}
		>
			<Typography.H3 className="p-2 text-center tracking-wider font-spicy-rice text-accent">
				{column.title}
			</Typography.H3>
