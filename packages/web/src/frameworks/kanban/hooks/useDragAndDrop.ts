...
export function useDragAndDrop({ onCardMove }: UseDragAndDropProps) {
	...
	useLayoutEffect(() => {
		if (dragState.isDragging && ghostRef.current) {
			if (dragState.draggedCardDimensions) {
				ghostRef.current.style.height = `${dragState.draggedCardDimensions.height}px`;
				ghostRef.current.style.width = `${dragState.draggedCardDimensions.width}px`;
			}
		}
