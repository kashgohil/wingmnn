...
export class ProjectService {
	...
	async createProject(
		...
		// Generate default key from project name if not provided
		// Take first 3 letters, uppercase, and remove non-alphabetic characters
		// Always ensure key is uppercase
		const defaultKey = data.key
			? data.key
					.toUpperCase()
					.replace(/[^A-Z]/g, "")
					.slice(0, 3) || null
